...
export const TableBlockNode: React.FC<NodeProps<TableBlockNodeData>> = ({ 
  id, 
  data, 
  selected 
}) => {
      ...
      <div className="relative">
        {data.headers.length > 0 ? (
              ...
              <table className="w-full border-collapse">
                ...
                <tbody>
                  {data.rows.map((row, rowIndex) => (
                    <tr
                      key={rowIndex}
                      className={`group ${getCellBackgroundColor(rowIndex)}`}
                    >
          ...
          >
            ...
            <p className="text-sm font-medium mb-1">Empty table</p>
            <p className="text-xs opacity-75 text-center">
              Select this block and configure table structure in the inspector
            </p>
          </div>
        )}
      </div>
