...
const FeaturedReview: React.FC<FeaturedReviewProps> = ({ review }) => {
  ...
  if (!review) {
    return (
      <div
        className={`w-full bg-surface rounded-md flex items-center justify-center ${isMobile ? 'h-64' : 'h-96'}`}
      >
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div
        className={`absolute inset-0 flex flex-col justify-center max-w-2xl ${isMobile ? 'p-4' : 'p-8'}`}
      >
        {/* Edition tag */}
        <div className="mb-4">
        ...
        {/* Title */}
        <h1
          className={`text-white font-bold leading-tight font-serif ${isMobile ? 'text-2xl mb-3' : 'text-4xl mb-4'}`}
        >
          {review.title}
        </h1>
        ...
        {review.description && (
          <p
            className={`text-white/90 leading-relaxed max-w-xl font-sans ${isMobile ? 'text-base mb-4' : 'text-lg mb-6'}`}
          >
            {review.description}
          </p>
        )}
        ...
        <button
          className={`bg-primary text-primary-foreground rounded-md font-semibold hover:bg-primary/90 transition-colors w-fit text-sm ${isMobile ? 'px-4 py-3 min-h-[44px]' : 'px-6 py-3'}`}
        >
          Ler agora
        </button>
      </div>
