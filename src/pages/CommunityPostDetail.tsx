...
const CommunityPostDetailContent = () => {
  ...
  const postId = React.useMemo(() => {
    ...
    if (!rawId) {
      console.error('CommunityPostDetail: No ID parameter found in route params');
      return 0;
    }
    
    const trimmedId = String(rawId).trim();
    if (!trimmedId) {
      console.error('CommunityPostDetail: Empty ID parameter after trim');
      return 0;
    }
    
    const parsed = parseInt(trimmedId, 10);
    if (isNaN(parsed) || parsed <= 0) {
      console.error('CommunityPostDetail: Invalid ID parameter:', trimmedId, 'parsed as:', parsed);
      return 0;
    }
    
    console.log('CommunityPostDetail: Valid ID parsed:', parsed);
    return parsed;
  }, [params]);

  console.log('CommunityPostDetail: Final postId for queries:', postId);
