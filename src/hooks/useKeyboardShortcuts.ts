...
export function useKeyboardShortcuts() {
  ...
  const matchesShortcut = useCallback((shortcut: KeyboardShortcut): boolean => {
  ...
  const shortcuts: KeyboardShortcut[] = React.useMemo(() => [
    ...
    },
    {
      ...
      action: () => {
        ...
        toast({
          title: "Redone",
          description: `Restored next state (${history.length - historyIndex - 1} actions ahead)`,
          duration: 1500,
        });
      },
    ...
    undo, redo, selectedNodeId, nodes, deleteNode, duplicateNode, copyNodes, pasteNodes,
    history, historyIndex, saveToDatabase, toggleFullscreen, toggleInspector, addNode,
    switchViewport, currentViewport, toast
  ]);
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    ...
    const matchingShortcut = shortcuts.find(shortcut => 
      !shortcut.disabled && matchesShortcut(shortcut)
    );
